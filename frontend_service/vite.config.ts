  ...
  export default defineConfig(({ mode }) => {
    ...
    if (mode === 'production') {
      ...
      if (!env.VITE_API_URL) {
        ...
        console.warn('   API calls will use relative paths which may not work in S3/CloudFront deployment.');
        console.warn('   Set VITE_API_URL to your API Gateway URL (e.g., https://api.bookstore.com)\n');
      } else if (!env.VITE_API_URL.startsWith('https://')) {
        console.warn('\n⚠️  WARNING: VITE_API_URL should use HTTPS in production.');
        console.warn(`   Current value: ${env.VITE_API_URL}\n`);
      }
    }
